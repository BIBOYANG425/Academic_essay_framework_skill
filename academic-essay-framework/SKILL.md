...
When the primary source is in the project knowledge PDFs, search those first. Use pdftotext or similar extraction tools to pull the text and confirm the exact wording. When the source is an outside scholar, verify against the primary text rather than a secondary summary.
...
When the primary source or course readings are stored as PDFs in the project knowledge, use the appropriate extraction approach to read them before building analysis. The docx skill at /mnt/skills/public/docx/SKILL.md should be used whenever the final output needs to be delivered as a Word document with double-spaced formatting, page numbers, and Times New Roman 12-point type, which is the standard expectation for humanities essays.
